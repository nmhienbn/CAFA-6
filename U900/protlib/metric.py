...
def iterate_from_df(df, G, batch_size, idx, back_idx, prop_mode='fill'):
    ...
    print(f" - Tổng số protein trong file dự đoán (sub.tsv): {sub_single['entry_id']}")
    print(f" - Tổng số protein có trong nhãn (validation): {idx}")
    
    # Thực hiện map
    sub_single['entry_num'] = sub_single['entry_id'].map(idx)
    
    # Đếm số lượng map thành công và thất bại
    n_total = len(sub_single)
    n_nan = sub_single['entry_num'].isna().sum()
    
    print(f" - Tổng số dòng dự đoán: {n_total}")
    print(f" - Số dòng bị NaN (không khớp ID): {n_nan}")
    
    if n_nan > 0:
        print(" -> PHÁT HIỆN LỆCH PHA DỮ LIỆU (ID Mismatch)!")
        print(" -> Ví dụ 5 ID trong file dự đoán nhưng KHÔNG có trong file nhãn:")
        # Lấy ra các ID bị lỗi để in
        error_ids = sub_single[sub_single['entry_num'].isna()]['entry_id'].unique()
        # Chuyển về numpy/list để in cho dễ nhìn (nếu đang dùng cudf)
        try:
            print(error_ids.to_pandas().head(5).tolist())
        except:
            print(error_ids.head(5))
...
class CAFAMetric:
    ...
    def calc_stats(self, batch, target, unique, G):
        ...
        mask_flg = merged['wpred'] > 0
        merged['flg'] = mask_flg

        # calc cov
        merged['bin_x'] = merged['bin'] + 1
        mtoi = merged.loc[mask_flg]

        # if len(mtoi) == 0:
        #     cov = cp.zeros(n_bins, dtype=cp.float32)
        # else:
        ...
        mtoi_small = mtoi[['entry_id', 'bin_x']]
        grp_max = mtoi_small.groupby('entry_id')['bin_x'].max()
        val_counts = grp_max.value_counts().to_frame()
        val_counts.columns = ['my_count']
        dummy_df = cudf.DataFrame([], index=cudf.RangeIndex(n_bins))
        cov_df = val_counts.join(dummy_df, how='right').sort_index().fillna(0)
        num_unique = mtoi['entry_id'].nunique()
        cov = num_unique - cov_df['my_count'].cumsum().values
        # ---
