...
parser.add_argument('-op', '--old-path', type=str)
parser.add_argument('-np', '--new-path', type=str)
parser.add_argument('-g', '--graph', type=str)
parser.add_argument('-t', '--target', type=str, required=True, help="Đường dẫn file chứa danh sách ID protein test (VD: test_ids.npy)")
...
if __name__ == '__main__':
    ...
    old = pd.read_csv(os.path.join(args.old_path, 'goa.tsv'), sep='\t', usecols=['EntryID', 'term']) \
        .drop_duplicates()
    ids = np.load(args.target)
    idxs = set(ids)
