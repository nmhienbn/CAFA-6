...
parser.add_argument('--run-index', type=int, default=-1,
                    help='Index của vòng lặp TTA cần chạy (0, 1, 2...). -1 là chạy hết.')
parser.add_argument('--batch-size', type=int, default=2048,
                    help='Batch size lớn cho A100')
parser.add_argument('--num-workers', type=int, default=32,
                    help='Số CPU workers load data')
...
if __name__ == '__main__':
    ...
    for nout, ontology in enumerate(['bp', 'mf', 'cc']):
        ...
        for k, tta_cfg in enumerate(nn_cfg['tta']):
            ...
            print(
                f"Running Inference: Ontology={ontology}, TTA_Index={k}, Config={tta_cfg}")

            output_path = os.path.join(
                config['base_path'], 'models/gcn', f'pred_tta_{k}.tsv')
            ...
            prior_cnd = joblib.load(
                os.path.join(
                    config['base_path'], f'helpers/real_targets/{G.namespace}/prior.pkl')
            )
            nulls = joblib.load(
                os.path.join(
                    config['base_path'], f'helpers/real_targets/{G.namespace}/nulls.pkl')
            )
            ...
            for folder, split, cnd in models_config:
                ...
                test_pred = joblib.load(path)[:, sum(
            ...
            for side_pred_name in nn_cfg['side_preds']:
                ...
                test_pred = side_data['test_pred'][:, sum(
                    split[:nout]): sum(split[:nout]) + split[nout]]
                idx = side_data['idx'][sum(split[:nout]): sum(
                    split[:nout]) + split[nout]]
                test_preds.append((test_pred, idx, False))

            test_goa_data = get_goa_data(os.path.join(
                config['base_path'], 'temporal', 'ver228'), 'test', test_idx, G)
