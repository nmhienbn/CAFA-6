...
if __name__ == '__main__':
    ...
    print(f"--- DEBUG INFO ({args.ontology}) ---")
    print(f"1. Tổng số dòng trong target ban đầu: {len(target)}")
    print(f"2. Root ID đang dùng để lọc: {root_id}")
    
    if root_id in target.columns:
        print(target[root_id])
        print(target[root_id].unique())
        num_positives = target[root_id].sum()
        print(f"3. Số lượng mẫu có root_id=1: {num_positives}")
        if num_positives == 0:
            print(">>> CẢNH BÁO: Không có mẫu nào có root_id=1! Kiểm tra lại dữ liệu ground truth.")
    else:
        print(f">>> LỖI: Cột {root_id} không tồn tại trong dữ liệu target!")
    ...
    for folder, split, cnd in models_config:
        ...
        if np.isnan(oof_pred).any():
            print(f"!!! CẢNH BÁO: Phát hiện NaN trong file {path}")
            print(f" - Số lượng NaN: {np.isnan(oof_pred).sum()}")
            # Tạm thời thay thế NaN bằng 0 để code chạy tiếp được
            oof_pred = np.nan_to_num(oof_pred, nan=0.0)
            print(" -> Đã thay thế NaN bằng 0.0")
        else:
            print(f"OK: File {path} sạch (không có NaN).")
