...
# Cần cài đặt node2vec: pip install node2vec
...
def main():
    ...
    if args.compute_node2vec:
        ...
        try:
            ...
            n2v = Node2Vec(G, dimensions=args.node2vec_dim, walk_length=10, num_walks=5, workers=4, quiet=True)
            model = n2v.fit(window=5, min_count=1, batch_words=4)
            # Lấy vector theo thứ tự nodes
            vectors = np.array([model.wv[str(n)] for n in nodes])
            # Lưu riêng hoặc gộp vào features (vì là matrix)
            # Ở đây ta lưu riêng vào npz key 'embedding'
        except ImportError:
